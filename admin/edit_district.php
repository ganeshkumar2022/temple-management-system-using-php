...
$obj=new district();
$state2=$obj->getdistrictById($_GET["id"]);
?>
...
$obj2=new state();
$states=$obj2->get_state();
foreach($states as $state)
{
    ?>
    <option value="<?=$state['sid']?>" <?php if($state2["state_id"]==$state["sid"]) { echo "selected"; } ?>><?=$state['state_name']?></option>
    <?php
}
?>
    </select>
  ...
  <div class="col">
      ...
      <input type="text" class="form-control state-bor" value="<?=$state2['district_name']?>" maxlength="25"  name="district" required id="state">
    </div>
    <div class="col"></div>
   </div>
  <button type="submit" class="btn button-color mt-3 text-white px-4 py-2" name="edit_district">Submit</button>
