...
$obj=new salary();
$states=$obj->get_salary_id($_GET["id"]);
?>
...
<div class="row">
    <div class="col">
      ...
      <input type="month" class="form-control state-bor" id="email" value="<?=$states['salary_month']?>" name="salary_month" required>
    </div>
    <div class="col">
...
if(mysqli_num_rows($result)>0)
{
    ...
    {
        ?>
        <option value="<?=$row['id']?>" <?php if($states["staff_id"]==$row['id']) { echo "selected"; } ?>><?=$row["staff_name"]?></option>
        <?php
    }
}
...
<div class="row mt-4">
    <div class="col">
      ...
      <input type="number" class="form-control state-bor" value="<?=$states['working_days']?>" id="email" name="working_days" required>
    </div>
    <div class="col">
    <label for="country">Per Day Salary</label>
      <input type="number" class="form-control state-bor" id="email" value="<?=$states['per_day_salary']?>"  name="per_day_salary" required>
    </div>
</div>
  <button type="submit" class="btn button-color mt-3 text-white px-4 py-2" name="edit_salary">Submit</button>
