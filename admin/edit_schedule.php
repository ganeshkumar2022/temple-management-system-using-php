...
$obj=new schedule();
$schedule=$obj->getSchedulebyId($_GET["id"]);
?>
<!-- schedule data get -->
...
if(mysqli_num_rows($result3)>0)
{
  ...
  {
    ?>
    <option value="<?=$row3['devotee_id']?>" <?php if($schedule["devotee_id"]==$row3["devotee_id"]) { echo "selected"; } ?> ><?=$row3["devotee_name"]?></option>
    <?php
  }
}
?>
...
if(mysqli_num_rows($result4)>0)
{
  ...
  {
    ?>
    <option value="<?=$row4['seva_id']?>" <?php if($schedule["seva_id"]==$row4["seva_id"]) { echo "selected"; } ?>><?=$row4["seva_name"]?></option>
    <?php
  }
}
?>
  ...
  <div class="row">
    <div class="col">
      ...
      <input type="date" class="form-control state-bor" id="email" value="<?=$schedule['date']?>" required name="rdate">
    </div>
    <div class="col">
    </div>
  </div>
  <button type="submit" class="btn button-color mt-3 text-white px-4 py-2" name="edit_schedule">Submit</button>
